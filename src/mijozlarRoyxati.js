...
function MijozlarRoyxati() {
  ...
  useEffect(() => {
    axios.get("https://risola-backend.onrender.com/users/getUsers")
      .then(res => {
  ...
  const handleSave = () => {
    ...
    axios.put(`https://risola-backend.onrender.com/users/updateUsersById/${editingUserId}`, formData)
      .then(res => {
  ...
  const handleDel = (user) => {
    if (window.confirm("❗ Bu mijozni o‘chirishga ishonchingiz komilmi?")) {
      axios.delete(`https://risola-backend.onrender.com/users/delete/${user._id}`)
        .then(() => {
